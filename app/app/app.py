...
def notify(
    ...
    try:
        ...
        print(f"Encountered an error while sending push notification: {error}")
...
def lambda_handler(context,
                   event):  # pylint: disable=inconsistent-return-statements, too-many-locals, too-many-branches, too-many-statements, unused-argument
    """
    ...
    if DEBUG:
        print(f"DEBUG: {CURRENT_TIME} - RSS feeds for user {USER}: {rss_feeds}")

    for feed_url in rss_feeds:
        if DEBUG:
            print(f"DEBUG: {CURRENT_TIME} - Checking feed {feed_url}")
        ...
        if feed.entries:
            ...
            if DEBUG:
                print(f"DEBUG: {CURRENT_TIME} - Feed {feed_url} contains {len(feed.entries)} entries")

            # Get the latest entry
            latest_entry = feed.entries[0]

            if DEBUG:
                print(f"DEBUG: {CURRENT_TIME} - Latest entry in feed {feed_url}: {latest_entry}")

            # Get the author of the tweet
            tweet_author = latest_entry.author

            if DEBUG:
                print(f"DEBUG: {CURRENT_TIME} - Author of latest entry: {tweet_author}")
            ...
            print(f"DEBUG: {CURRENT_TIME} - Feed {feed_url} is empty or invalid.")
